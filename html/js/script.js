(function($) {
    ...
    'use strict';
      ...
      WeddingCity_Scroll_Div( 'goto_request_quote', 'request_quote_form' );
      ...
      if( $( '#texonomy_pagination' ).length ){
          ...
          $('#texonomy_pagination').twbsPagination({
              ...
              // Text labels
              first: false,
              prev: WC_AJAX_OBJ.pagination_prev_text ? WC_AJAX_OBJ.pagination_prev_text : 'Previous',
              next: WC_AJAX_OBJ.pagination_next_text ? WC_AJAX_OBJ.pagination_next_text : 'Next',
              last: false,
      ...
      if( $('.videosection').length ){
          ...
          $('.videosection').c47bg({
              type: 'youtube',
              container: 'div',
              source: $('.videosection').attr( 'data-src' ),
              crop: true
          });
      }
  
      // @link -  https://stackoverflow.com/questions/6539502/this-serialize-how-to-add-a-value#question-header
  
      //. https://gist.github.com/jonsuh/3739844
  
      // var data = {
              
      //         'action'            :   'weddingcity_vendor_profile_update',
      //         'post_content'      :   get_editor_value( '.note-editable'),
      //         'security'          :   $( '#weddingcity_vendor_profile_update #profile_update').val()
      // };
      ...
      // if( $( 'ul#wc-search-listing' ).length ){
  
      //     $( document ).on( 'click', 'ul#wc-search-listing li a', function( e ){
